<template>
    <div class="container">
        <div class="row" style="margin-top: 6%;">
            <h2>Поиск категорий</h2>
            <div class="col">
                <form>
                    <input class="form-control" v-model="search" type="search" placeholder="Поиск категорий">
                </form>
            </div>
        </div>
    </div>
    ...
    <div class="container">
        <div class="row mb-5" v-if="searchProductFilter.length === 0">
            <div class="alert alert-danger" role="alert">
                <h2>Поиск по категориями не найдено!</h2>
            </div>
        </div>
      ...
      computed: {
          searchProductFilter(){
              return this.products.filter(el => {
                  return el.category.title.includes(this.search);
              });
          }
      },
      async beforeMount(){
